...
interface NavigationProps { menuData: MenuCategory[]; }
...
const Navigation: React.FC<NavigationProps> = ({ menuData }) => {
  ...
  return (
    <nav className="relative mb-8 w-full">
      <div className="pointer-events-none absolute left-0 top-0 h-full w-10 bg-gradient-to-r from-background to-transparent sm:hidden"/>
      ...
      <div ref={scrollRef}
           className="flex items-center gap-3 overflow-x-auto whitespace-nowrap py-3 px-8 scroll-smooth scrollbar-hide snap-x snap-mandatory">
        {menuData.map(cat => (
          <a key={cat.id} href={`#${cat.id}`} onClick={() => setActiveId(cat.id)}
             className={`inline-block px-6 py-2 font-medium text-sm rounded-full snap-start transition-all duration-300
                        ${activeId === cat.id ? "bg-[#9A8C98] text-[#F2E9E4] shadow-[0_0_10px_#9A8C98]"
                                              : "bg-[#22223B] text-[#F2E9E4] hover:bg-[#4A4E69]"} 
